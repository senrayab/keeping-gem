...
- 장소(`lib/kakao.ts`): 입력 화면에서 카카오맵으로 공연장을 찾아 고르면 좌표가 함께 저장되고,
  상세보기에서 장소를 누르면 지도(`MapViewer.tsx`)가 열린다. SDK는 쓸 때만 불러오고,
  못 불러와도(비행기 모드 등) 장소 이름은 그대로 쓴다. **카카오 JavaScript 키는 도메인 제한으로 보호된다.**
- 머리말 오른쪽 **설정**(`BackupSheet.tsx`, `lib/backup.ts`): 저장 현황과 zip 백업 저장 / 합쳐서 불러오기.
  (머리말 아이콘 순서: 리스트 · 사진첩 · 설정)
