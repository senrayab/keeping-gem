...
import { db, type Poster, type Ticket } from '@/db/db'
 ...
 *
 *   backup.json          앱 이름·형식 버전·만든 때·티켓 목록(그림 제외)
...
export async function createBackup(): Promise<{ blob: Blob; name: string; count: number }> {
  ...
  const manifest: BackupFile = { app: APP, format: FORMAT, createdAt: Date.now(), tickets: stored }
...
export async function readBackup(file: File): Promise<RestorePreview> {
  ...
  return { tickets, posters, createdAt: manifest.createdAt ?? 0, existing }
}
 ...
 */
export async function applyBackup({ tickets, posters }: RestorePreview): Promise<void> {
